...
export default class Correlation {
  ...
  correlationCoefficient() {
    return (
      this.diffFromAvg() /
      Math.sqrt(this.diffFromAvgSqrd(this.x) * this.diffFromAvgSqrd(this.y))
    );
  }
  ...
  diffFromAvg() {
    ...
    return this.x.reduce(
      (carry, item, i) => carry + (item - avg_x) * (this.y[i] - avg_y),
      0,
    );
  }
  ...
  diffFromAvgSqrd(list) {
    return list.reduce(
      (carry, item) => carry + Math.pow(item - this.avg(list), 2),
      0,
    );
  }
   ...
   */
  sumXTimesY() {
    return this.x.reduce((carry, item, i) => carry + this.y[i] * item, 0);
  }
